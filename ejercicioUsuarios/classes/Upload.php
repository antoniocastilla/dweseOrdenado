...
/*
Falta por hacer:
[X]1. si un archivo no se sube por POLICY_KEEP se debe devolver un error para ello
   error = 3
[X]2. el propio move_uploaded_file() puede fallar, por no tener permiso de escritura,
   porque no existe la ruta de destino, porque ya se ha movido, etc.
   error = 4
[X]3. guardar el nombre con el que se ha guardado el archivo (POLICY_RENAME)

Resumen de errores:
1 - no existe el archivo que se va a subir
2 - el tamaño del archivo excede el máximo, o el tipo no es el valido
...
class Upload {
    ...
    function isValidType() {
        ...
        if($this->type !== '') {
            ...
            //Strpos primera ocurrencia de un subtring en un string
            if($posicion === false) {
                $valid = false;
            }
        }
